...
namespace eevm
{
  ...
  std::map<Address, std::pair<SimpleAccount, SimpleStorage>> SimpleGlobalState::get_all_accounts() {
    return accounts;
  }

  void SimpleGlobalState::set_block(Block b) {
    currentBlock = b;
  }

  void SimpleGlobalState::set_accounts(std::map<Address, std::pair<SimpleAccount, SimpleStorage>> acc) {
    accounts = acc;
  }
