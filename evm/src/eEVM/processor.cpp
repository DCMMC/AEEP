...
namespace eevm
{
  ...
  void Processor::init(Address sender) {
	...
	long end_time = clock();
	if (CAL_LOAD_STORE_TIME) printf("load store time: %f\n", ((double)end_time - start_time) / 1000);
  }
  ...
  exec_result_struct Processor::run(
	...
	long end_time = clock();
	if (CAL_CREATE_ENCLAVE_TIME) printf("create enclave time: %f\n", ((double)end_time - start_time) / 1000);
	// ==================== enclave create ====================
	...
	memcpy(input, vector2arr(_input), sizeof(uint8_t) * _input.size());

	unsigned char* call_value = uint2562charx(_call_value);

	exec_result_struct result;
	result.exmsg = new unsigned char[100];
	result.output = new uint8_t[3000];
	result.output_size = uint2562charx(0);

	// ==================== enclave run ====================
	start_time = clock();
	enclave_run(enclave, &result, &gs, &tx, caller, callee, input, _input.size(), call_value);
	end_time = clock();
	if (CAL_RUNNING_TIME) printf("enclave run time: %f\n", ((double)end_time - start_time) / 1000);
	// ==================== enclave run ====================
  ...
  Address Processor::deploy(
	...
	// ==================== enclave run ====================
	enclave_deploy(enclave, &gs, &tx, caller_address, constructor, _constructor.size(), contract_address);
	// ==================== enclave run ====================
	...
	if (enclave != NULL)
	{
	  oe_result_t oe_result = oe_terminate_enclave(enclave);
	}
	// ==================== enclave terminate ====================
