...
unsigned char keykey[KEY_LEN] = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f,
                                      0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f };

void aes256_enc(unsigned char* data, unsigned char* res) {
  ByteArray key;
  for (unsigned char i = 0; i < KEY_LEN; i++)
    key.push_back(keykey[i]);

  ByteArray enc;

  Aes256::encrypt(key, data, TEXT_SIZE, enc);

  memset(res, 0, sizeof(unsigned char) * 200);
  memcpy(res, &enc[0], sizeof(unsigned char) * ENC_SIZE);
}

void aes256_dec(unsigned char* data, unsigned char* res) {
  ByteArray key;
  for (unsigned char i = 0; i < KEY_LEN; i++)
    key.push_back(keykey[i]);
  ...
  Aes256::decrypt(key, data, ENC_SIZE, dec);

  memset(res, 0, sizeof(unsigned char) * 200);
  memcpy(res, &dec[0], sizeof(unsigned char) * TEXT_SIZE);
}